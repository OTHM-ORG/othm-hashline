...
LIBRARY_FILE = libothm_hashline.so
HEADER_FILE = othm_hashline.h

all: $(LIBRARY_FILE)

$(LIBRARY_FILE): bin/ $(OBJ_FILES) src/$(HEADER_FILE)
...
test:
	gcc -g -Wall -o test test.c -lothm_hashline -lothm_base
	./test
install:
